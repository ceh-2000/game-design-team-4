...
void Music_Player::decrementPos(float p)
{
	if((this->music.getPlayingOffset() - sf::seconds(p)) <= sf::seconds(0.0f)) 
		this->music.setPlayingOffset(sf::seconds(0.0f));	
	else 
		this->music.setPlayingOffset( sf::Time(this->music.getPlayingOffset() - sf::seconds(p))); 
}

void Music_Player::incrementPos(float p)
{
	if((this->music.getPlayingOffset() + sf::seconds(p))>=this->music.getDuration())
		this->music.setPlayingOffset(this->music.getDuration());
	else 
		this->music.setPlayingOffset( this->music.getPlayingOffset() + sf::seconds(p));
}
