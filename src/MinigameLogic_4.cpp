...
MinigameLogic_4::MinigameLogic_4(std::shared_ptr<Song> song) {
	...
	for (float time : timings) {
		if (iter == 4) iter = 0;
		if (iter < 4) {
			beat_box = std::make_shared<BeatBoxLogic>(sf::Vector2f(75 * iter + 825, 800.0f),
									sf::Vector2f(75 * iter + 825, 100.0f),
									sf::Vector2f(75 * iter + 825, 34.0f),
									velocity, time);
...
void MinigameLogic_4::setScore(const int& input, const int &tapCheck) {
	switch (tapCheck) {
...
std::string MinigameLogic_4::gradeMinigame() {
	...
	float grade =  this->score/(40*beatBoxes.size()) * 100.0f;
